...
This This portfolio comprises **more than twenty quantitative trading and finance projects**, ranging from real‑data market simulations to advanced stochastic volatility models, portfolio optimisation routines, risk‑management tools, machine‑learning classifiers, reinforcement‑learning agents and even a high‑performance C++ matching engine. Each project is self‑contained, heavily commented and designed to illustrate a specific concept in modern quantitative trading. Some scripts operate on real market data (with synthetic fallbacks), while others generate realistic synthetic data to showcase algorithm design. To run any project, ensure you have Python 3.8+ (or a C++ compiler for the matching engine) and install the required packages listed at the top of each script (typically `pandas`, `numpy`, `scipy`, `scikit‑learn`, `matplotlib` and optionally `yfinance`).  You can install them via:
```bash
